...
void callService(ServiceRequest &req, ServiceResponse &res){
    ...
    waitForResponse(id);
    res = slots[id].res;
    printf("Length: %u,  Alpha: %u,  Numbers: %u,  ServerId: %u\n",res.len,res.alpha,res.numbers,res.serverid);fflush;
...
void *client(void *arg){
    ...
    for (i = 0; i < iter; i++)
    {
        ...
        //printf(req.frase);
        callService(req, res);
        //printf("resposta %u , %u , %u",res.alpha,res.len,res.numbers);
        //printf("Resposta: %d",res.serverid);fflush;
    }
    return NULL;
}
