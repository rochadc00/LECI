...
// necessidade de criar isto para que seja possível ler a informação de cada paciente e
// proceder ao seu atendiemnto de forma individual e sucessiva
static pthread_t *patientLife; 
static pthread_t *nursesLife;
static pthread_t* doctoresLife;
static pthread_mutex_t *patientMutex;  // Exclusive access for each patient
static pthread_cond_t *patientReady;   // End of consultation for that patient, ready to go
...
void init_simulation(uint32_t np, u_int32_t nn, uint32_t nd)
{
   ...
   patientLife = new pthread_t[np];
   nursesLife = new pthread_t[nn];
   doctoresLife = new pthread_t[nd];
   patientMutex = new pthread_mutex_t[np];
   patientReady = new pthread_cond_t[np];

   for (uint32_t i = 0; i < np; i++) {
         mutex_init(&patientMutex[i],NULL);
         cond_init(&patientReady[i],NULL);
   }
...
void* doctor_life(void *arg) {
   while(1) {
      ...
      mutex_lock(&patientMutex[patient]);
      hd->all_patients[patient].done = 1;
      cond_broadcast(&patientReady[patient]);
      mutex_unlock(&patientMutex[patient]);
   } return NULL;
}
...
{
   mutex_lock(&patientMutex[id]); // Exclusive access to the patient

   while( hd->all_patients[id].done == 0) {
      cond_wait (&patientReady[id], &patientMutex[id]);
   }

   check_valid_name(hd->all_patients[id].name);
   printf("\e[30;01mPatient %s (number %u): health problems treated\e[0m\n", hd->all_patients[id].name, id);
   
   mutex_unlock(&patientMutex[id]);
}
...
int main(int argc, char *argv[])
{
   ...
   for (uint32_t i=0; i < nnurses; i++) {
      thread_create(&nursesLife[i],NULL, nurse_life, NULL);
   }

   for (uint32_t i=0; i < ndoctors; i++) {
      thread_create(&doctoresLife[i],NULL, doctor_life, NULL);
   }

   for (uint32_t i=0; i < npatients; i++) {
      int *id = (int *)malloc(sizeof(int));
      *id = i;
      thread_create(&patientLife[i],NULL, patient_life, (void *)id);
   }

   for (uint32_t i=0; i < npatients; i++) {
      thread_join(patientLife[i],NULL);   
   }

   // for (uint32_t i=0; i < ndoctors; i++) {
   //    thread_detach(doctoresLife[i]);
   // }
   // for (uint32_t i=0; i < nnurses; i++) {
   //    thread_detach(nursesLife[i]);
   // }
   ...
   // for(uint32_t i = 0; i < npatients; i++)
   // {
   //    cond_destroy(&patientReady[i]);
   //    mutex_destroy(&patientMutex[i]);
   // }
   // free(hd);
