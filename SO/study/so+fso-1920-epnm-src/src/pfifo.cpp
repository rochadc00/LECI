...
{  
   mutex_init(&pfifo->accessCR, NULL);   // concede acesso à leitura, mudificações
   cond_init(&pfifo->notFull, NULL);   // fifo não está cheia no 
   cond_init(&pfifo->notEmpty,NULL);   // fifo não está vazia

   mutex_lock(&pfifo -> accessCR);  // Get exclusive access to the FIFO
   ...
   cond_broadcast(&pfifo-> notFull);   // Broadcast to wake all threads waiting for the FIFO to be FULL
   mutex_unlock(&pfifo -> accessCR);   // Release exclusive access to the FIFO
}
...
void insert_pfifo(PriorityFIFO* pfifo, uint32_t id, uint32_t priority)
{
   ...
   while (full_pfifo(pfifo)) {
      cond_wait(&pfifo -> notFull, &pfifo-> accessCR); // wait that someone says that is not full anymore
